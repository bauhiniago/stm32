...
void LCD_HZ(uint16_t x,uint16_t y,uint8_t num,uint8_t size,uint8_t mode){
  uint8_t temp,t1,t;
  uint16_t y0=y;
  //uint8_t csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数
  for(t=0;t<128;t++)
  {
    if(size==32)temp=HZK32[num][t];	//调用汉字库字体
    else return;								//没有的字库
    for(t1=0;t1<8;t1++)
    {
      if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
void LCD_HZS(uint16_t x,uint16_t y,uint8_t *num,uint8_t nmax,uint8_t size,uint8_t mode){
  uint8_t n;
  for (n=0;n<nmax;n++){
    LCD_HZ(x+(n*32),y,num[n],size,mode);
  }
}
/*
void LCD_HZS_MOVE(uint16_t x,uint16_t y,uint8_t *num,uint8_t nmax,uint8_t size,uint8_t mode,uint8_t dir){
  uint16_t x0;
  uint16_t y0;
  x0=x;
  y0=y;
  LCD_HZS(30,100,(uint8_t*)hznum,30,32,0);
}
*/
