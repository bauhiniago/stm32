...
uint8_t GT9147_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_11,GPIO_PIN_RESET);  //复位F11
  HAL_Delay(10);
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_11,GPIO_PIN_SET);//释放复位
  HAL_Delay(10);
...
void TP_Write_Byte(uint8_t num)
{
  ...
  {
    if(num&0x80)HAL_GPIO_WritePin(GPIOF,GPIO_PIN_9,GPIO_PIN_SET);
    else HAL_GPIO_WritePin(GPIOF,GPIO_PIN_9,GPIO_PIN_RESET);
    num<<=1;
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);
    delay_us(1);
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET);	//上升沿有效
  }
}
...
uint16_t TP_Read_AD(uint8_t CMD)
{
  uint8_t count=0;
  uint16_t Num=0;
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);;		//先拉低时钟
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_9,GPIO_PIN_RESET);; 	//拉低数据线
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_11,GPIO_PIN_RESET);; 		//选中触摸屏IC
  TP_Write_Byte(CMD);//发送命令字
  delay_us(6);//ADS7846的转换时间最长为6us
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);;
  delay_us(1);
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET);;		//给1个时钟，清除BUSY
  delay_us(1);
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);;
  for(count=0;count<16;count++)//读出16位数据,只有高12位有效
  {
    Num<<=1;
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);;	//下降沿有效
    delay_us(1);
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET);;
    if(HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_2))Num++;
  }
  Num>>=4;   	//只有高12位有效.
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_11,GPIO_PIN_SET);;		//释放片选
  return(Num);
}
...
void CT_IIC_Start(void)
{
  ...
  CT_Delay();
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);//钳住I2C总线，准备发送或接收数据
}
//产生IIC停止信号
void CT_IIC_Stop(void)
{
  ...
  CT_Delay();
  HAL_GPIO_WritePin(GPIOF,GPIO_PIN_9,GPIO_PIN_SET);//发送I2C总线结束信号
}
//等待应答信号到来
//返回值：1，接收应答失败
//        0，接收应答成功
uint8_t CT_IIC_Wait_Ack(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);//时钟输出0
  return 0;
}
//产生ACK应答
...
void CT_IIC_Send_Byte(uint8_t txd)
{
  uint8_t t;
  CT_SDA_OUT();
  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);//拉低时钟开始数据传输
  CT_Delay();
  for(t=0;t<8;t++)
  {
    HAL_GPIO_WritePin(GPIOF,GPIO_PIN_9,(txd&0x80)>>7);
    txd<<=1;
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET);
    CT_Delay();
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET);
