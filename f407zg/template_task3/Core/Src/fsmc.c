...
void LCD_Scan_Dir(uint8_t dir)
{
  ...
  uint16_t dirreg=0;
  uint16_t temp;
  if((lcddev.dir==1&&lcddev.id!=0X1963)||(lcddev.dir==0&&lcddev.id==0X1963))//横屏时，??1963不改变扫描方向！竖屏??1963改变方向
  {
    switch(dir)//方向转换
    {
      case 0:dir=6;break;
...
void LCD_Display_Dir(uint8_t dir)
{
  ...
  if(dir==0)			//竖屏
  {
    ...
    {
      lcddev.wramcmd=0X2C;	//设置写入GRAM的指??
...
//设置窗口(对RGB屏无??),并自动设置画点坐标到窗口左上??(sx,sy).
//sx,sy:窗口起始坐标(左上??)
//width,height:窗口宽度和高??,必须大于0!!
//窗体大小:width*height.
...
//初始化lcd
//该初始化函数可以初始化各种型号的LCD(详见??.c文件??前面的描??)
void LCD_Init(void)
{
  ...
  if(lcddev.id!=0X9341)		//??9341,尝试看看是不是NT35310
  {
    ...
    if(lcddev.id!=0X5310)		//也不是NT35310,尝试看看是不是NT35510
    {
      ...
      LCD_WR_REG(0XDC00);
      lcddev.id|=LCD_RD_DATA();		//读回0X00
      if(lcddev.id==0x8000)lcddev.id=0x5510;//NT35510读回的ID??8000H,为方便区??,我们强制设置??5510
      if(lcddev.id!=0X5510)			//也不是NT5510,尝试看看是不是SSD1963
      {
        ...
        if(lcddev.id==0X5761)lcddev.id=0X1963;//SSD1963读回的ID??5761H,为方便区??,我们强制设置??1963
      }
    }
  }
  ...
  //初始化完成以??,提???
  if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最??
  {
    //重新配置写时序控制寄存器的时??
    FSMC_Bank1E->BWTR[6]&=~(0XF<<0);//地址建立时间(ADDSET)清零
    FSMC_Bank1E->BWTR[6]&=~(0XF<<8);//数据保存时间清零
    FSMC_Bank1E->BWTR[6]|=3<<0;		//地址建立时间(ADDSET)??3个HCLK =18ns
    FSMC_Bank1E->BWTR[6]|=2<<8; 	//数据保存时间(DATAST)??6ns*3个HCLK=18ns
  }
  LCD_Display_Dir(0);		//默认为竖??
